...
%
% Answer: Remove items from workspace, freeing up system memory
%
...
%
% Answer: fact (computes the factorial of a number and returns the result) 
%
...
%
% Answer: While Loops- Repeats a statement or group of statements while 
% a given condition is true.
% For Loops- Executes a sequence of statements multiple times and 
...
%
% Answer: Stands for not a number. Expresses undefined values
% NaN occurs when you have expressions like 0/0 and inf/inf, and can also
% appear when we load data with missing values. 
%
...
% 12. Create a variable my age and store your age. Subtract two from the value of the
...
minusThree = vec-(3) 
countPositives = length(nonzeros(vec(vec>0)))
absoluteVal = abs(vec) 
maxNum = max(vec) 
...
trapArea= .5*(base1+base2)*height 

%Comment: 
% trapArea is the desired output (area of the trapezoid). The formula for
% calculating the area is area = .5*(base1+base2)*height. I
% first assigned the variables (base1, base2, height) values then 
% plugged the variables into the formula and stored the result in trapArea. 
